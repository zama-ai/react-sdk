...
describe("useShield", () => {
    const wrapperAddress = "0xabcdef1234567890abcdef1234567890abcdef12";
    ...
    describe("callbacks", () => {
        it("should call onError when shield fails", async () => {
            const onError = vi.fn();
            const { result } = renderHook(() => useShield({ wrapperAddress, underlyingAddress, onError }), { wrapper: DisconnectedWrapper });
            await act(async () => {
                try {
                    await result.current.shield(100n);
                }
    ...
    describe("memoization", () => {
        it("should maintain stable shield function reference", () => {
            const { result, rerender } = renderHook(() => useShield({ wrapperAddress, underlyingAddress }), { wrapper: ConnectedWrapper });
...
//# sourceMappingURL=useShield.test.js.map