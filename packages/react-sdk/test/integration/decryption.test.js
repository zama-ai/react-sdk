...
function createMockFhevmInstanceForDecryption() {
    ...
    const publicDecryptMock = vi.fn().mockImplementation(async (handles) => {
        const clearValues = {};
 ...
 */
function createDecryptionWrapper(instance, provider = createMockProvider()) {
    ...
    return function DecryptionWrapper({ children }) {
...
describe("Decryption Integration Tests", () => {
    ...
    describe("usePublicDecrypt integration", () => {
        ...
        it("should decrypt multiple handles in batch", async () => {
            const mockInstance = createMockFhevmInstanceForDecryption();
            const wrapper = createDecryptionWrapper(mockInstance);
            const { result } = renderHook(() => usePublicDecrypt({ handles: [MOCK_HANDLE, MOCK_HANDLE_2] }), { wrapper });
            await act(async () => {
                result.current.decrypt();
            });
            await waitFor(() => {
        ...
        it("should filter out undefined handles", async () => {
            const mockInstance = createMockFhevmInstanceForDecryption();
            const wrapper = createDecryptionWrapper(mockInstance);
            const { result } = renderHook(() => usePublicDecrypt({ handles: [undefined, MOCK_HANDLE, undefined] }), { wrapper });
            expect(result.current.canDecrypt).toBe(true);
            await act(async () => {
                result.current.decrypt();
            });
    ...
    describe("useUserDecrypt integration", () => {
        it("should return canDecrypt based on params", async () => {
            ...
            const { result } = renderHook(() => useUserDecrypt({ handle: MOCK_HANDLE, contractAddress: CONTRACT_ADDRESS }), { wrapper });
            expect(result.current.canDecrypt).toBe(true);
        });
        it("should not be able to decrypt without handle", async () => {
            const mockInstance = createMockFhevmInstanceForDecryption();
            const wrapper = createDecryptionWrapper(mockInstance);
            const { result } = renderHook(() => useUserDecrypt({ handle: undefined, contractAddress: CONTRACT_ADDRESS }), { wrapper });
            expect(result.current.canDecrypt).toBe(false);
        });
        it("should not be able to decrypt without contract address", async () => {
            const mockInstance = createMockFhevmInstanceForDecryption();
            const wrapper = createDecryptionWrapper(mockInstance);
            const { result } = renderHook(() => useUserDecrypt({ handle: MOCK_HANDLE, contractAddress: undefined }), { wrapper });
            expect(result.current.canDecrypt).toBe(false);
        });
    ...
    describe("decryption state management", () => {
        it("should show decrypting status during operation", async () => {
            ...
            mockInstance.publicDecrypt.mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve({
...
//# sourceMappingURL=decryption.test.js.map