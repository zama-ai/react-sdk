...
const WRAPPER_ADDRESS = "0x9876543210987654321098765432109876543210";
const UNDERLYING_ADDRESS = "0x5555555555555555555555555555555555555555";
 ...
 */
function createERC7984Wrapper(instance, provider = createMockProviderForERC7984()) {
    ...
    return function ERC7984Wrapper({ children }) {
...
describe("ERC7984 Integration Tests", () => {
    ...
    describe("useConfidentialBalances integration", () => {
        it("should initialize with correct state", () => {
            ...
            const { result } = renderHook(() => useConfidentialBalances({
        ...
        it("should handle multiple contracts", () => {
            const mockInstance = createMockFhevmInstanceForERC7984();
            const wrapper = createERC7984Wrapper(mockInstance);
            const secondToken = "0x1111111111111111111111111111111111111111";
            const { result } = renderHook(() => useConfidentialBalances({
    ...
    describe("useConfidentialTransfer integration", () => {
        it("should initialize with idle state", () => {
            const mockInstance = createMockFhevmInstanceForERC7984();
            const wrapper = createERC7984Wrapper(mockInstance);
            const { result } = renderHook(() => useConfidentialTransfer({ contractAddress: TOKEN_ADDRESS }), { wrapper });
            expect(result.current.status).toBe("idle");
        ...
        it("should support custom ABI", () => {
            ...
            const { result } = renderHook(() => useConfidentialTransfer({
    ...
    describe("useUnshield integration", () => {
        ...
        it("should support callbacks", () => {
            ...
            const onSuccess = vi.fn();
            const onError = vi.fn();
            const { result } = renderHook(() => useUnshield({
                wrapperAddress: WRAPPER_ADDRESS,
                onSuccess,
                onError,
            }), { wrapper });
    ...
    describe("full ERC7984 flow simulation", () => {
        it("should support shield -> balance check flow", () => {
            ...
            const wrapper = createERC7984Wrapper(mockInstance);
            // First, shield tokens
            const { result: shieldResult } = renderHook(() => useShield({
                wrapperAddress: WRAPPER_ADDRESS,
                underlyingAddress: UNDERLYING_ADDRESS,
            }), { wrapper });
            expect(shieldResult.current.status).toBe("idle");
            // Then, check balance
            const { result: balanceResult } = renderHook(() => useConfidentialBalances({
        ...
        it("should support transfer -> balance check flow", () => {
            ...
            const { result: transferResult } = renderHook(() => useConfidentialTransfer({ contractAddress: TOKEN_ADDRESS }), { wrapper });
...
//# sourceMappingURL=erc7984.test.js.map